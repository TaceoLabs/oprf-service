...
start_keygen_node() {
    ...
    TACEO_OPRF_KEY_GEN_BIND_ADDR="0.0.0.0:${port}" \
    TACEO_OPRF_KEY_GEN_SERVICE_WALLET_PRIVATE_KEY="${node_private_keys[$i]}" \
    TACEO_OPRF_KEY_GEN_SERVICE_ENVIRONMENT=dev \
    TACEO_OPRF_KEY_GEN_SERVICE_ZKEY_PATH=./artifacts/OPRFKeyGen.13.arks.zkey \
    TACEO_OPRF_KEY_GEN_SERVICE_WITNESS_GRAPH_PATH=./artifacts/OPRFKeyGenGraph.13.bin \
    TACEO_OPRF_KEY_GEN_SERVICE_OPRF_KEY_REGISTRY_CONTRACT="$DEPLOYED_ADDRESS" \
    TACEO_OPRF_KEY_GEN_SERVICE_CONFIRMATIONS_FOR_TRANSACTION=1 \
    TACEO_OPRF_KEY_GEN_SERVICE_EXPECTED_THRESHOLD="$THRESHOLD" \
    TACEO_OPRF_KEY_GEN_SERVICE_EXPECTED_NUM_PEERS="$NUM_PEERS" \
    TACEO_OPRF_KEY_GEN_SERVICE_WS_RPC_URL="$WS_RPC_URL" \
    TACEO_OPRF_KEY_GEN_SERVICE_RPC_HTTP_URLS="$RPC_URL" \
    TACEO_OPRF_KEY_GEN_SERVICE_RPC_CHAIN_ID=31337 \
    TACEO_OPRF_KEY_GEN_SERVICE_BACKFILL_SKIP_BACKFILL="${SKIP_BACKFILL:-no}" \
    TACEO_OPRF_KEY_GEN_SERVICE_BACKFILL_CONFIRMATIONS_AFTER_SYNC_BLOCK=2 \
    TACEO_OPRF_KEY_GEN_SERVICE_BACKFILL_CHUNK_SIZE=2 \
    TACEO_OPRF_KEY_GEN_POSTGRES_CONNECTION_STRING="$POSTGRES_URL" \
    TACEO_OPRF_KEY_GEN_POSTGRES_SCHEMA="$schema" \
    "${dd_env[@]+"${dd_env[@]}"}" \
...
start_oprf_service_nodes() {
    ...
    for i in $(peer_indices); do
        ...
        env \
        RUST_LOG="taceo=trace,warn" \
        TACEO_OPRF_NODE_POSTGRES_CONNECTION_STRING="$POSTGRES_URL" \
        TACEO_OPRF_NODE_POSTGRES_SCHEMA="${schemas[$i]}" \
        TACEO_OPRF_NODE_SERVICE_ENVIRONMENT=dev \
        TACEO_OPRF_NODE_SERVICE_VERSION_REQ=">=0.0.0" \
        TACEO_OPRF_NODE_SERVICE_STORE_TTL=0s \
        TACEO_OPRF_NODE_SERVICE_STORE_TTI=0s \
        TACEO_OPRF_NODE_BIND_ADDR="0.0.0.0:${port}" \
        TACEO_OPRF_NODE_NODE_URLS="$node_urls_csv" \
        "${dd_env[@]+"${dd_env[@]}"}" \
